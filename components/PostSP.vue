<template>
  <div class="post">
    <div class="header">
      <PostTime :time="post.date" :linkid="post.id" />
      <nuxt-link :to="`/topic/${post.topic_id}`" class="topic-link">{{
        post.topic_name
      }}</nuxt-link>
      <a
    ...
    <div class="post-wrap">
      ...
      <nav class="main-nav">
        <nuxt-link :to="`/user/${post.author}`" class="username"
          >{{ post.author }}
          <p
            v-if="isOP"
            class="op-badge"
        ...
        >
        <nuxt-link :to="`/user/${post.author}`">
          <img
            :src="`${backendURL}/api/user/${post.author}/picture`"
            width="90"
            height="90"
            class="pfp"
          />
        ...
        <PostCount :user="post.author" />
        <Status :user="post.author" style="display: block" />
      </nav>
    </div>
  </div>
</template>
